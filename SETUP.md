...
| Cact.cz | 季節オファー(PDF)は10月開始で、現時点ではファイル自体が未公開のため後回し |

### 未着手(追加の仕組みが必要)

- **SuccSeed**: JS依存のSPAのため、ヘッドレスブラウザ(Playwright等)の導入が必要。Cheerioでは対応できず実装が煩雑・処理時間も長くなるため保留
...
9サイト合計で約21,400商品を取得し、うち**1,769商品**が保護種と確定マッチ(**119種**に何らかの購入先あり)。
